 ...
 */
export function DateOfBirthPicker({ id, label, value, onChange, season, error }: Props) {
  const { t } = useTranslation("apply");
  const [open, setOpen] = React.useState(false);

  const { min, max, defaultMonth } = React.useMemo(() => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {value ? format(parseISO(value), "d MMMM yyyy 'г.'", { locale: bg }) : t("fields.dateOfBirthPlaceholder")}
          </Button>
        </PopoverTrigger>
      ...
      {error && <p id={`${id}-error`} role="alert" className="text-xs font-semibold text-destructive">{t(error)}</p>}
    </div>
  );
}
